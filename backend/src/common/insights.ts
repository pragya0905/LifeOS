...
// Shared by the on-demand /insights endpoint (getInsights) and the automatic weekly digest
// (weeklyInsightsScheduler) — one place to build the AI context so both stay in sync.
export async function generateInsightsForUser(
  ...
  const [habits, medicationLogs, routineLogs, logEntries, expenses, medications, routines, tasks] =
    await Promise.all([
      ...
      queryByDateRange<Expense>(process.env.EXPENSES_TABLE_NAME as string, userId, from, to),
  ...
  if (logEntries.length > 0) {
    lines.push("Other logs (food, sleep, weight, mood, cycle, calls):");
  ...
  if (expenses.length > 0) {
    ...
    lines.push(`Expenses (₹${total} total):`);
    for (const e of expenses) {
      lines.push(`- ${e.date} [${e.category}]: ₹${e.amount}${e.note ? ` (${e.note})` : ""}`);
    }
  }
  ...
  return generateInsights(period, lines.join("\n"));
}
