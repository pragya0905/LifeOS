...
// Mirrors listMedicationLogsForRange — lets a client fetch several days of routine step
// logs in one query instead of one request per day, used for streak calculation.
export const handler: APIGatewayProxyHandlerV2WithJWTAuthorizer = async (event) => {
  ...
  const result = await ddb.send(
    new QueryCommand({
      ...
      KeyConditionExpression: "userId = :userId AND dateRoutineStep BETWEEN :from AND :to",
